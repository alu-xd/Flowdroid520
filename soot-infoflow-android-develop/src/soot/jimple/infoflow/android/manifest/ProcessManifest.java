...
public class ProcessManifest {
	...
	//罗丹添加方法
			public Set<String> getEntryPointClasses(String entrypointskind) 
			{
			...
			//罗丹添加方法
			public Set<String>[] getEntryPointClassesset() 
			{
				Set<String> entrypointsset[]=new Set[5];
				// If the application is not enabled, there are no entry points
				if (!isApplicationEnabled())
				{
					entrypointsset[0]=Collections.emptySet();
					entrypointsset[1]=Collections.emptySet();
					entrypointsset[2]=Collections.emptySet();
					entrypointsset[3]=Collections.emptySet();
					entrypointsset[4]=Collections.emptySet();
					return entrypointsset;
				}
				
				// Collect the components
				entrypointsset[0] = new HashSet<String>();
				entrypointsset[1] = new HashSet<String>();
				entrypointsset[2] = new HashSet<String>();
				entrypointsset[3] = new HashSet<String>();
				entrypointsset[4] = new HashSet<String>();
				
				for (AXmlNode node : this.activities)
					checkAndAddComponent(entrypointsset[0], node);
				for (AXmlNode node : this.receivers)
					checkAndAddComponent(entrypointsset[1], node);
				for (AXmlNode node : this.providers)
					checkAndAddComponent(entrypointsset[2], node);
				for (AXmlNode node : this.services)
					checkAndAddComponent(entrypointsset[3], node);
				
				String appName = getApplicationName();
				if (appName != null && !appName.isEmpty())
					entrypointsset[4].add(appName);
