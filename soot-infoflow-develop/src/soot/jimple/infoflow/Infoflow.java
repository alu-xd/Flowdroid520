...
public class Infoflow extends AbstractInfoflow {
    ...
    //fly selectSource control 
    private static boolean selfDefineMod=false;
    private static Map<String,Set<String>> selectSource=new HashMap<String,Set<String>>();
    
	public static void setSource(Map<String,Set<String>> sources){
...
//		forwardSolver.setEnableMergePointChecking(true);
		...
		if(config.getConfig().isJustPrintresultnum())
		{
			System.out.println("There are about "+res.size() +" path. ----\n "
					+ "And if you want to know the detail about it,"
					+ "please don't use the --justprintresultnum args "
					+ "and make sure there is enough memory for this kind of running config!");
			maxMemoryConsumption = Math.max(maxMemoryConsumption, getUsedMemory());
		...
		else if(config.getConfig().isUsingChange())
		{
			System.out.println("Don't use the icfg path!");
	...
	private int scanMethodForSourcesSinks(
			...
			SootMethod m) {
		//fly get name of location method
		final String mn = m.toString();
		//fly
		...
		if (m.hasActiveBody()) {
			...
			for (Unit u : units) {
				...
				if(s.containsInvokeExpr())
					sm=s.getInvokeExpr().getMethod().getSignature();
				else
					sm=s.toString();
				if (sourcesSinks.getSourceInfo(s, iCfg) != null) {// fly 依旧在这里处理 过滤
					if((!selfDefineMod)||(selectSource.containsKey(sm)&&
							selectSource.get(sm).contains(mn))){
						forwardProblem.addInitialSeeds(u, Collections.singleton(forwardProblem.zeroValue()));
						if (getConfig().getLogSourcesAndSinks())
							collectedSources.add(s);
						logger.debug("Source found: {}", u);
